...
export class CreateFurnitureManagerHandler implements ICommandHandler<CreateFurnitureManagerCommand> {
    constructor(
    ...
    async execute(command: CreateFurnitureManagerCommand): Promise<any> {
        let furnitureManagerId: number = 0;
        FurnitureManager.setInstance(FurnitureManagerId.create(command.id),command.name);
        let furnitureManager = FurnitureManager.getInstance();
        ...
        furnitureTypeORM = await this._furnitureManagerRepository.save(furnitureTypeORM);
