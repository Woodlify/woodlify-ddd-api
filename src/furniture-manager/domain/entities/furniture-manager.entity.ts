...
export class FurnitureManager extends AggregateRoot {
  ...
  private constructor(private _id: FurnitureManagerId, private _name: string) {
    super();
    this._furnitures = [];
  }
  ...
  public register() {
    const event = new FurnitureManagerCreated(this._name, this._furnitures);
    this.apply(event);
  }
  ...
  public deleteFurnitureId(id: number, name: string): void {
    ...
    const event = new DeletedDesignEvent(id, name);
    this.apply(event);
  }
